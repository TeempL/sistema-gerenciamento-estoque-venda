...
import com.techsolutions.gestaovenda.modelo.util.AbstractMouseListener;
import com.techsolutions.gestaovenda.modelo.dao.CategoriaDao;
import com.techsolutions.gestaovenda.modelo.dao.ClienteDao;
import com.techsolutions.gestaovenda.modelo.dao.ProdutoDao;
import com.techsolutions.gestaovenda.modelo.dao.UsuarioDao;
import com.techsolutions.gestaovenda.modelo.dao.VendaDao;
...
import com.techsolutions.gestaovenda.modelo.entidades.Venda;
import com.techsolutions.gestaovenda.modelo.entidades.VendaDetalhes;
import com.techsolutions.gestaovenda.modelo.exception.NegocioException;
import com.techsolutions.gestaovenda.modelo.util.VendaRegistroTableModel;
import com.techsolutions.gestaovenda.modelo.util.VendaTableModel;
...
public class VendaController extends AbstractMouseListener implements ActionListener, KeyListener {
    ...
    private VendaDetalhes vendaDetalhe;
    ...
    private void removerProdutoNoCesto() {
        if(this.nomeDoProduto != null && !this.nomeDoProduto.isEmpty()) {
            ...
            mensagemNaTela("Deve selecionar o produto que desejas remover", Color.RED);
        }
    }
        ...
        if(autenticacaoDao.temPermissao(usuario)){
            if(this.venda == null) {
                int confirmar = JOptionPane.showConfirmDialog(dashboard, 
                        String.format("Certeza que deseja apagar? \nid: %d", this.venda.getId()), 
                        "Apagar venda", JOptionPane.YES_NO_OPTION);
                
                    if(confirmar == JOptionPane.YES_OPTION) {
                        String mensagem = vendaDao.deleteVendaPeloId(this.vendaDetalhe.getVenda().getId());
                        JOptionPane.showMessageDialog(dashboard, mensagem);
                        limparCampo();
                    }
            } else {
                JOptionPane.showMessageDialog(dashboard, "Deve selecionar uma venda na tabela", "Seleciona uma venda", 0);
            }
        }
    }
